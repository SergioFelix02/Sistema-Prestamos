        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <form method="POST" action="{{ route('montos.store') }}">
                        @csrf
                        <label class="block text-sm font-bold mb-2 dark:text-white text-black">Agregar Monto:</label>
                        <textarea
                            name="monto"
            ...
            <div class="mt-6 p-6 bg-white dark:bg-gray-800 shadow-sm rounded-lg divide-y dark:divide-gray-900 dark:text-white text-black">
                <table id="myTable" class="display table-hover table-sm text-center">
                    <caption>Registro de Montos</caption>
                    <thead>
                        <tr>
                            <th class="col-sm-1">ID</th>
                            <th class="col-sm-1">Montos</th>
                        ...
                        <tr>
                            ...
                            <td class="no-datatables">
                                <button style="margin-right: 10px;">
                                    <a href="{{ route('montos.edit', $monto) }}">
                                ...
                                <button>
                                    <form method="POST" action="{{ route('montos.destroy', $monto) }}">
                                        @csrf @method('DELETE')
                                        <a href="{{ route('montos.destroy', $monto) }}" onclick="event.preventDefault(); this.closest('form').submit();">
                                            <svg class="w-5 h-5 text-gray-500 dark:text-gray-300" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
